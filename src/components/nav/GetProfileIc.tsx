interface GetProfileIcProp {
  activeNav: string;
  width: number;
  hover: string
}
const GetProfileIc = ({ activeNav, width, hover }: GetProfileIcProp) => {
  return (
    <>
      <svg
      ...
      >
        <path
          fill={activeNav === "profile" || hover === 'profile' ? "#633CFF" : "#737373"}
      ...
      {width >= 768 && (
        <span
          className={`font-semibold ${
            activeNav === "profile" || hover === 'profile' ? "text-purple-300" : "text-grey-200"
          }`}
        >
          Profile Details
        </span>
