interface GetLinkIcProp {
  activeNav: string;
  width: number;
  hover: string
}
const GetLinkIc = ({ activeNav, width, hover }: GetLinkIcProp) => {
  return (
    <>
      <svg
      ...
      >
        <path
          fill={activeNav === "home" || hover === "home" ? "#633CFF" : "#737373"}
      ...
      {width >= 768 && (
        <span
          className={`font-semibold ${
            activeNav === "home" || hover === 'home' ? "text-purple-300" : "text-grey-200"
          }`}
        >
          Links
        </span>
